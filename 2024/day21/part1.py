...
INPUT = {
    ...
    # For each input, the strings are instructions we must type into a directional keypad using another directional keypad
    # Multiple possible optimal instructions per input. 
...
for numeric, options in INPUT.items():
    ...
    for base in options:
        ...
        prev = "A"
        for c in base:
            result += type_using_directional(prev, c, 1)
            prev = c
