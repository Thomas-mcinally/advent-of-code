...
int main(int argc, char **argv)
{
    ...
    for (int i=0; i<linecount; i++)
    {
        ...
        if (five_of_a_kind) arrput(buckets[0], lines[i]);
        else if (four_of_a_kind) arrput(buckets[1], lines[i]);
        else if (three_of_a_kind && two_of_a_kind) arrput(buckets[2], lines[i]);
        else if (three_of_a_kind) arrput(buckets[3], lines[i]);
        else if (two_of_a_kind == 2) arrput(buckets[4], lines[i]);
        else if (two_of_a_kind) arrput(buckets[5], lines[i]);
        else arrput(buckets[6], lines[i]);
    }
